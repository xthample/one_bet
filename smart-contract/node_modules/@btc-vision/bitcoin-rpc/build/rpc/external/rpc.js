...
export var GetBlockTemplateMode;
(function (GetBlockTemplateMode) {
    GetBlockTemplateMode["TEMPLATE"] = "template";
    GetBlockTemplateMode["PROPOSAL"] = "proposal";
})(GetBlockTemplateMode || (GetBlockTemplateMode = {}));
const voidMethods = [
    'invalidateblock',
    'reconsiderblock',
    'preciousblock',
...
export class RPCClient extends RESTClient {
    ...
    async getblockBatch(blockhashes, verbosity = 1) {
        try {
    ...
    async getblockhashes(heights, count) {
        try {
            const response = await this.batch(Array.from({ length: count }, (_, i) => {
                return {
                    method: 'getblockhash',
    ...
    generateblock({ output, transactions = [], submit = true }) {
        return this.rpc('generateblock', { output, transactions, submit });
    }
    addpeeraddress({ address, port, tried = false }) {
        return this.rpc('addpeeraddress', { address, port, tried });
    }
    sendmsgtopeer({ peer_id, msg_type, data }) {
        return this.rpc('sendmsgtopeer', { peer_id, msg_type, data });
    }
    submitpackage({ package: pkg, maxfeerate, maxburnamount }) {
    ...
    psbtbumpfee(options, wallet) {
        return this.rpc('psbtbumpfee', options, wallet || this.wallet);
    }
    send({ outputs, conf_target, estimate_mode, fee_rate, options }, wallet) {
        return this.rpc('send', { outputs, conf_target, estimate_mode, fee_rate, options }, wallet || this.wallet);
    }
    sendall({ recipients, conf_target, estimate_mode, fee_rate, options }, wallet) {
        return this.rpc('sendall', { recipients, conf_target, estimate_mode, fee_rate, options }, wallet || this.wallet);
    }
    simulaterawtransaction({ rawtxs = [], options = {} } = {}, wallet) {
        return this.rpc('simulaterawtransaction', { rawtxs, options }, wallet || this.wallet);
    }
    upgradewallet({ version } = {}, wallet) {
    ...
    async getrawtransactionBatch(txids, verbose = true) {
        try {
